...
Index of project documentation. Start with [README.md](../README.md) for install and overview. Product version **2.0.2** is `APP_VERSION` in `apps/desktop/src/views/about.rs` (bump after each release).
...
7. Bump `APP_VERSION` in `apps/desktop/src/views/about.rs` after each product release (currently **2.0.2**).

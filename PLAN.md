...
Product version is `APP_VERSION` in `apps/desktop/src/views/about.rs` and `[package.metadata.packager] version` in `apps/desktop/Cargo.toml` (currently **2.0.5**). The Cargo workspace crate version is independent — do not bump it for a product release.
